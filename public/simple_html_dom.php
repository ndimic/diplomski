 ...
 * is the "real" number of bytes the dom was created from. but for most purposes, it's a really good estimation. Paperg
 * - Added the forceTagsClosed to the dom constructor.  Forcing tags closed is great for malformed html, but it CAN
 * lead to parsing errors. Allow the user to tell us how much they trust the html. Paperg add the text and plaintext to
 * the selectors for the find syntax.  plaintext implies text in the innertext of a node.  text implies that the tag is
 * a text node. This allows for us to find tags based on the text they contain. Create find_ancestor_tag to see if a
 * tag is - at any level - inside of another specific tag. Paperg: added parse_charset so that we know about the
 * character set of the source document. NOTE:  If the user's system has a routine called
