            ...
            <div class="row">
                ...
                <div class="col-md-6">
                    ...
                    @if($car->created_at->diffInMinutes() > 60 && $car->created_at->diffInHours() < 24)
                        <h3 align="center" class="border" style="background-color: #fff; color: black;">Postavljeno: pre {{ $car->created_at->diffInHours() }} sata.</h3>
                    @elseif($car->created_at->diffInHours() > 24)
                        <h3 align="center" class="border" style="background-color: #fff; color: black;">Postavljeno: pre {{ $car->created_at->diffInDays() }} dana.</h3>
                    @else
                        <h3 align="center" class="border" style="background-color: #fff; color: black;">Postavljeno: pre {{ $car->created_at->diffInMinutes() }} minuta.</h3>
                    @endif
                    @if($car->status)
                        <h3 align="center">Status: </h3><h5 align="center" style="color: forestgreen;">Odobren</h5>
                    @else
                        <h3 align="center">Status: </h3><h5 align="center" style="color: red;">Neodobren (čeka se obrada admina)</h5>
                    @endif
                </div>
            </div>
